...
{ //F2
use          "Data_public_pre_reform.dta", clear 
g ay1	    = ned if ned<2*365
g ay4       = wg_c if ned<2*365
local   ii = 3
g     agec = (int(age*`ii')/`ii')+1/(2*`ii')
...
(qfit    y    age   if   age<40 , lcolor(black) ) ///
, xline(40, lp(dash) lcolor(gs10)) graphregion(fcolor(white)) legend(off)  ///
ytitle("Non-employment duration (days)") xtitle("Age at layoff") subtitle("Panel a",size(small) ) name(F2A, replace)
cap drop y m
g y = ay4
egen m  = mean(y), by(agec)
twoway ///
...
(qfit    y    age   if   age>=40, lcolor(black) ) ///
(qfit    y    age   if   age<40 , lcolor(black) ) ///
, xline(40, lp(dash) lcolor(gs10)) graphregion(fcolor(white)) legend(off)  ytitle("Wage change between jobs") ///
...
(qfit    y    age   if   age>=40, lcolor(black) ) ///
(qfit    y    age   if   age<40 , lcolor(black) ) ///
, ytitle("`tit'") xline(40, lp(dash) lcolor(gs10)) graphregion(fcolor(white)) legend(off)  xtitle("Age at layoff") subtitle("Panel d",size(small) ) name(F3d, replace) 
